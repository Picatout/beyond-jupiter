...
/*******************************
    kbd_scancode 
    extract scancode from queue 
******************************/
    _HEADER scan_code,5,"SCODE" 
    _PUSH 
    ldr XP,[UP,#KBD_QHEAD_OFS]
1:  ldr YP,[UP,#KBD_QTAIL_OFS]
    cmp XP,YP
    beq 1b
    add YP,UP,#KBD_QUEUE_OFS
    ldrb TOS,[YP,XP]
    add XP,#1 
    and XP,#KBD_QUEUE_SIZE-1 
    strb XP,[UP,#KBD_QHEAD_OFS]
    _RET 
...
// video buffer address 
	_HEADER VIDEO_BUFFER,8,"VID-BUFF"
	_PUSH 
	ldr TOS,[UP,#VID_BUFFER_OFS]
	_NEXT 
...
QUIT2:
	...
	BL	EVAL
	BL	BRAN
	.word	QUIT2	// continue till error
